...
abstract class CI_DB_query_builder extends CI_DB_driver {
	...
	{
		if (empty($this->qb_orderby)) { return ''; }
		for ($i = 0, $c = count($this->qb_orderby); $i < $c; $i++)
		{
			if (is_string($this->qb_orderby[$i])) { continue; }
	...
	protected function _object_to_array_batch($object)
	{
		...
		foreach ($fields as $val)
		{
			...
			{
				$i = 0;
				foreach ($out[$val] as $data) { $array[$i++][$val] = $data; }
			}
		}
		return $array;
	}
	...
	{
		if (count($this->qb_cache_exists) === 0) { return; }
